...
import EmailBannerTriggerEditor from './triggers/EmailBannerTriggerEditor.jsx';
...
class Editor extends React.Component {
    ...
    componentWillMount () {
        ...
        LocalStorageStore.set( "dashboardTab", Constants.PRODUCT_EMAIL_BANNER );
    ...
    onLoadComplete () {
        ...
        if (this.props.location.query.validateAssist) {
            let validationErrors = CampaignValidation.validateEmailBanner(this.state.currentCampaignDetails, this.state.currentCampaign.status);
    ...
    saveAndPreview () {         
        ...
        let validationErrors = CampaignValidation.validateEmailBanner(this.state.currentCampaignDetails, this.state.currentCampaign.status, true);
        ...
        if (validationErrors.length === 0) {
            ...
            let previewUrl = PreviewUtils.getDemoStorePreviewUrl( Constants.PRODUCT_EMAIL_BANNER, this.state.campaignHash, true, UserStore.isFeatureAuthorized(Constants.PRODUCT_EMAIL_BANNER,"activate") );
    ...
    saveAndActivate () {
        ...
        if (validationErrors.length === 0) {
            this.setState( newState, () => {
                ...
                setTimeout( () => {
                    this.setState( { saving: false, dirty: false }, () => {
                        if (UserStore.isFeatureAuthorized(Constants.PRODUCT_EMAIL_BANNER,"activate")) {
                            this.activator.activate( this.state.currentCampaign, Constants.PRODUCT_EMAIL_BANNER );
                        } else {
    ...
    onUpdateEveryoneWinsPrizes ( prizes, giveawayEvents, secretSave ) {
        this.updatePrizes( prizes, giveawayEvents, "giveawayEvents", undefined, secretSave );
    }


    updatePrizes ( prizes, prizeEvents, eventKey, loss, secretSave ) {
        ...
        // We do this because we are forcing an update to the campaign without showing the user (done for creating default prizes)
        ...
        return (
            <div>
                <ActionBar 
                    ...
                    featureAuth={UserStore.isFeatureAuthorized(Constants.PRODUCT_EMAIL_BANNER,"activate")}
                />
                ...
                { ConfigStore.getEditorSectionEnabled("giveaway") ?
                    <EveryoneWins 
                        ...
                        defaultPrizes={ [
                            {
                                type: "incentive",
                                title: ConfigStore.getTemplateSkinDefaultPrize(this.state.currentCampaignDetails.themeInfo.layout, this.state.currentCampaignDetails.themeInfo.name)
                            }
                        ] }
                        campaignDetails={this.state.currentCampaignDetails}
                        campaignHash={this.state.campaignHash}
                ...
                { CampaignStore.isUnscheduled(this.state.currentCampaign.status) ?
                    <div className="editor-call-to-action">
                        ...
                        {
                            UserStore.isFeatureAuthorized(Constants.PRODUCT_EMAIL_BANNER,"activate") ? 
