...
class CampaignCard extends React.Component {
    ...
    onCampaignStatsRetrieved ( e ) {
        if ( e.response && e.response.hasErrors() ) {
            ...
            if (e.response.modifiedResult[this.props.campaignHash]) {
                ...
                //this.setState( { stats: e.response.modifiedResult[this.props.campaignHash] }, this.checkLoaded.bind(this) );
            }
        }
    }
    ...
    onEdit () {
        History.push(
            ConfigStore.buildRoutePath('edit/'+this.props.campaignHash, this.props.product)
        );
    }
    onDelete () {
    ...
    getCardButtons () {
        if (CampaignStore.isComplete(this.props.status)) {
            ...
            return <div className="card-ctrl">
                <button className="card-button" onClick={this.onEdit.bind(this)}><i className="material-icons">edit</i>{i18n.stringFor("sh_label_edit")}</button>
                <button className="card-button" onClick={this.onView.bind(this)}><i className="material-icons">search</i>{i18n.stringFor("sh_label_preview")}</button>
                <button className="card-button" onClick={this.onActivate.bind(this)}><i className="material-icons">power_settings_new</i>{UserStore.isFeatureAuthorized(Constants.PRODUCT_EMAIL_BANNER,"activate") ? i18n.stringFor("sh_label_activate") : i18n.stringFor("sh_label_subscribe_to_activate")}</button>
                { ConfigStore.getPermission("campaign","clone") ? <button className="card-button" onClick={this.onClone.bind(this)}><i className="material-icons">content_copy</i>{i18n.stringFor("sh_label_clone")}</button> : null }
                { ConfigStore.getPermission("campaign","delete") ? <button className="card-button" onClick={this.onDelete.bind(this)}><i className="material-icons">delete</i>{i18n.stringFor("sh_label_delete")}</button> : null }
            </div>
        }
            ...
            <div className="social-card">
                <div className="card-info">
                    ...
                    <div className="card-content">
                        <div className="card-preview-landscape card-preview">
                            <img className="img-banner" src={"/dashboard/images/dashboard/bannerpreview_"+ this.props.campaignDetails.themeInfo.layout +".jpg"} onClick={this.onEdit.bind(this)}/>
                        </div>
