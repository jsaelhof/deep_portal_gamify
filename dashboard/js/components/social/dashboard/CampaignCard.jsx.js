...
class CampaignCard extends React.Component {
    ...
    onLiveView () {
        window.open(CampaignUtils.getMainLink(this.props.campaignDetails),"_blank");
    }
    onEdit () {
        History.push(
        ...
        if (this.props.campaignDetails.selectedGame) {
            if (CampaignStore.isComplete(this.props.status)) {
                return <div className="card-ctrl">
                    ...
                    <button className="card-button" onClick={this.onLiveView.bind(this)}><i className="material-icons">search</i>{i18n.stringFor("sh_label_live_view")}</button>
                    { ConfigStore.getPermission("campaign","clone") ? <button className="card-button" onClick={this.onClone.bind(this)}><i className="material-icons">content_copy</i>{i18n.stringFor("sh_label_clone")}</button> : null }
                ...
                return <div className="card-ctrl">
                    ...
                    <button className="card-button" onClick={this.onActivate.bind(this)}><i className="material-icons">power_settings_new</i>{UserStore.isFeatureAuthorized(Constants.PRODUCT_SOCIAL,"activate") ? i18n.stringFor("sh_label_activate") : i18n.stringFor("sh_label_subscribe_to_activate")}</button>
                    { ConfigStore.getPermission("campaign","clone") ? <button className="card-button" onClick={this.onClone.bind(this)}><i className="material-icons">content_copy</i>{i18n.stringFor("sh_label_clone")}</button> : null }
                    { ConfigStore.getPermission("campaign","delete") ? <button className="card-button" onClick={this.onDelete.bind(this)}><i className="material-icons">delete</i>{i18n.stringFor("sh_label_delete")}</button> : null }
                </div>
            }
                ...
                <div className="card-info">
                    <div className="card-header">
                        ...
                        <div className="card-status">
                            ...
                            { this.props.campaignDetails.startDate && this.props.campaignDetails.endDate ?
                                <div className="status-date">{TimezoneStore.getMomentInTimezone(this.props.campaignDetails.startDate,this.props.campaignDetails.timezone).format(CampaignStore.fullDateDisplayFormat)} - {TimezoneStore.getMomentInTimezone(this.props.campaignDetails.endDate,this.props.campaignDetails.timezone).format(CampaignStore.fullDateDisplayFormat)}</div>
                                :
                                "Unscheduled"
                            }
                            {
                                this.props.status === CampaignStore.STATUS_SCHEDULED ?
                                    <div className="status-time">Begins {TimezoneStore.getMomentInTimezone(this.props.campaignDetails.startDate,this.props.campaignDetails.timezone).fromNow()}</div>
                                    :
                                    null
                            }
                            {
                                this.props.status === CampaignStore.STATUS_RUNNING ?
                                    <div className="status-time">Ends {TimezoneStore.getMomentInTimezone(this.props.campaignDetails.endDate,this.props.campaignDetails.timezone).fromNow()}</div>
                                    :
                                    null
                            }
                        </div>
